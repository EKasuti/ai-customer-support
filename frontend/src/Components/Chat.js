...
const ChatComponent = ({ onClose }) => {
  ...
  const formatResponse = (content) => {
    ...
    // Ensure list items are on their own lines with line breaks
    content = content.replace(/(\d+)\.\s/g, '\n$1. ');
    // Move text after colon to the next line
    ...
    try {
      ...
      while (true) {
        ...
        setMessages((messages) => {
          ...
          return [
            ...otherMessages,
            { ...lastMessage, content: formatResponse(lastMessage.content + text) },
          ];
        });
      }
    } catch (error) {
